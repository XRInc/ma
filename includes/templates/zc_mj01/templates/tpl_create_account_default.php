<div class="create-account container">
    ...
    <form id="form-validate" method="post" action="<?php echo href_link(FILENAME_CREATE_ACCOUNT, '', 'SSL'); ?>">
		...
		<div class="row">
			...
			<div class="form-group col-sm-6">				<label class="required" for="customer-telephone"><em>*</em><?php echo __('*Telephone'); ?></label>				<input type="text" class="form-control input-text required-entry validate-telephone" name="customer-telephone" id="customer-telephone" />			</div>
		</div>
		<div class="checkbox">			<label for="customer-newsletter">				<a class="item text-content" href="<?php echo href_link(FILENAME_CMS_PAGE, 'cpID=3'); ?>">				<input type="checkbox" class="checkbox" title="<?php echo __('Agree to user agreement or privacy policy & agree to receive SMS'); ?>" value="1"<?php if (isset($customer['newsletter'])&&$customer['newsletter']==1) { ?> checked="checked"<?php } ?> name="customer[newsletter]" id="customer-newsletter" />				<?php echo __('Agree to user agreement or privacy policy & agree to receive SMS'); ?>				</a>			</label></div>
