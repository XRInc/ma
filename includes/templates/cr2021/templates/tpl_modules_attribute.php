...
<?php
	...
	switch ($_option['type']) {
		...
		case 'list' :
			...
			foreach ($_option['value'] as $_optionValue) {
				$optionValuePriceHtml = $_optionValue['price'] > 0 ? ' ( ' . $_optionValue['price_prefix'] . $currencies->display_price($_optionValue['price']) . ' )' : '';
				$optionValueHtml .= '<li onclick="$(\'#attribute_' . $_option['product_option_id'] . '\').val('.$_optionValue['product_option_value_id'].');" class="active">' . __($_optionValue['name']) . $optionValuePriceHtml . '</li>';
			}
			$optionValueHtml .= '</ul>';
			$optionValueHtml .= '<input type="hidden" class="' . $optionvalueRequired . '" value="'.$_optionValue['product_option_value_id'].'" name="attribute[' . $_option['product_option_id'] . ']" id="attribute_' . $_option['product_option_id'] . '" />';
		break;
		case 'wholesale' :
