...
// 数据库配置CLOAK_API常量初始化
$configuration = $db->Execute("SELECT configuration_key, configuration_value FROM " . TABLE_CONFIGURATION . " WHERE configuration_key LIKE 'CLOAK%'", '', true, 604800);
...
{
	if(CLOAKERLY_SWITCH == 0)
	{
		$isZp = 1;
	}
	else if(CLOAKERLY_SWITCH == 2)
	{
		require_once "init_cloakerly.php";
		$check = new CloakerlyChecker();
		if (isset($_COOKIE['ip_check_cloak_json']))
		{
			$ipCloakCheckData = json_decode($_COOKIE['ip_check_cloak_json'], true);
			if( !$check->ForceRedirect("status",$ipCloakCheckData) )
			{
				$isZp = 1;
			}
		}
		...
		{
		    $check->SetKey("nSyqtbKQ5uq8ZFj9ms7wd6UWNVSUQozaYpl0BxRKoefhErriB1DkCfxZ12JAsIOg");
		    $check->SetCampaign("3832");
		    $check->SetStrictness("0");
		    $check->SetReferrer(isset($_SERVER["HTTP_REFERER"]) ? $_SERVER["HTTP_REFERER"] : "Direct");
		    $check->PassUserAgent(true);
		    if( !$check->ForceRedirect("status") )
		    {
		    	$isZp = 1;
		    }
		}
